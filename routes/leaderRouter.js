...
.all((req,res,next) => {            //for all requests whether it its is get, put post or delete
 ...
 .put((req,res,next)=>{            
     ...
     res.end('PUT operation not supprted on leaders');
  })
 ...
 .get((req,res,next)=> {
    res.end('Will send the detalis of the leader: '+ req.params.leaderId + 'to you!');
})

.post((req,res,next)=>{             
    res.statusCode=403;                    //403 means operation not supported
    res.end('POST operation not supprted on /leaders/' + req.params.leaderId);
})
